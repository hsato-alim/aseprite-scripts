...
function ShadingMode:Process(change, sprite, lastCel, options)
    if not change.pixels or #change.pixels == 0 then
        return
    end
    ...
    -- local leftButtonPressed = change.leftPressed -- Old logic based on GetButtonsPressed
    -- local rightPressed = change.rightPressed -- Old logic
    local tolerance = options.shadingTolerance or 0

    -- Determine shiftAmount using the new ColorContext function based on painted hue
    if not change.pixels[1] or not change.pixels[1].newColor then
      return
    end
    local paintedColor = change.pixels[1].newColor
    local shiftAmount = ColorContext:GetShiftAmountFromHue(paintedColor)

    if shiftAmount == 0 then
        -- This means hue was ambiguous (e.g. grayscale) or paintedColor was nil.
        -- No action will be taken.
        return
    end
    -- Note: The original lines for setting shiftAmount based on leftButtonPressed/rightButtonPressed
    -- and the subsequent `if shiftAmount == 0 then return end` are now replaced by the logic above.
    ...
    if sourceIndexInPalette == -1 then
        return
    end
    ...
    for _, pixel in ipairs(change.pixels) do
        ...
        if currentIndexInPalette ~= -1 then
            ...
            if options["rampCheck" .. currentRampNumber] then
                ...
                if tolerance == 0 then
                    -- Explicitly handle tolerance 0: only the exact sourceIndexInPalette should be considered.
                    isWithinTolerance = (currentIndexInPalette == sourceIndexInPalette)
                else
                ...
                if isWithinTolerance then
                    ...
                    if newRampNumber == currentRampNumber and newIndexInPalette >= 0 and newIndexInPalette < #palette then
                        ...
                        if newColor and not ColorContext:IsTransparent(ColorContext:Create(newColor)) then
                            local targetX = pixel.x - lastCel.position.x
                            local targetY = pixel.y - lastCel.position.y
                            drawPixel(image, targetX, targetY, newColor)
